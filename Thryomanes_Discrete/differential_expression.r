...
dds_og <- dds
rm("dds")
metadata <- read.csv('/n/holylfs06/LABS/edwards_lab/Lab/maxlaubstein/climatechangeRNA/Thryomanes/Nest_Box_RNA.csv', header = TRUE, sep = ",", check.names = FALSE)
...
metadata <- metadata %>% select("id", "Tissue", "Treatment", "Sex", Individual = "Catalog Number")
...
keep <- rowSums(counts(dds) > 0) >= 3 # only keep data points where the gene count is greater than 0 in at least 3 samples 
dds <- dds[keep,]
dds <- DESeq(dds)
